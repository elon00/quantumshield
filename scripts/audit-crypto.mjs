...
try {
  ...
  console.log('\n▶ [TIER 2] Canonical Hash & Commitment Invariants:');
  ...
  const testPayload = Buffer.from('QuantumShield Root Commitment');
  const commitment = Buffer.from(sha256(testPayload)).toString('hex');
  assert.strictEqual(commitment.length, 64);
  pass('State commitment produces exact 32 bytes hex');

  console.log('\n▶ [TIER 3] NIST FIPS 203 ML-KEM-768 Lattice Execution:');
  const seed64 = new Uint8Array(64).fill(0x7a);
  ...
  const seed32 = new Uint8Array(32).fill(0x5c);
  ...
  const msg = Buffer.from('QuantumShield Core Auditor Signature Verification Payload');
  ...
  const hybridKey = hkdf(sha256, Buffer.concat([classicalSecret, decSharedSecret]), new Uint8Array(32), Buffer.from('QuantumShield-Hybrid'), 32);
  assert.strictEqual(hybridKey.length, 32);
  pass('Dual hybrid key material derives 256-bit symmetric session key');

  const badHybridKey = hkdf(sha256, Buffer.concat([classicalSecret, implicitKey]), new Uint8Array(32), Buffer.from('QuantumShield-Hybrid'), 32);
  assert.notDeepStrictEqual(Buffer.from(hybridKey), Buffer.from(badHybridKey));
